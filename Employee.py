...
class EmployeeClass:
    def __init__(self, root):
        ...
        cmb_Gender = ttk.Combobox(self.root, textvariable=self.var_gender_id, values=("Select", "Male", "Female", "Other"), state="readonly", justify=CENTER, font=("goudy old style", 15))
        cmb_Gender.place(x=500, y=150, width=180)
        cmb_Gender.current(0)
        txt_contact = Entry(self.root, textvariable=self.var_contact_id, font=("goudy old style", 15), bg="lightyellow").place(x=780, y=150)

        # Row 2
        lbl_name = Label(self.root, text="Name", font=("goudy old style", 15), bg="white").place(x=50, y=190)
        lbl_dob = Label(self.root, text="D.O.B", font=("goudy old style", 15), bg="white").place(x=350, y=190)
        lbl_doj = Label(self.root, text="D.O.J", font=("goudy old style", 15), bg="white").place(x=683, y=190)

        txt_Name = Entry(self.root, textvariable=self.var_name_id, font=("goudy old style", 15), bg="lightyellow").place(x=150, y=190, width=180)
        txt_dob = Entry(self.root, textvariable=self.var_dob_id, font=("goudy old style", 15), bg="lightyellow").place(x=500, y=190, width=180)
        txt_doj = Entry(self.root, textvariable=self.var_doj_id, font=("goudy old style", 15), bg="lightyellow").place(x=750, y=190, width=180)

        # Row 3
        lbl_email = Label(self.root, text="Email", font=("goudy old style", 15), bg="white").place(x=50, y=230)
        lbl_pass = Label(self.root, text="Password", font=("goudy old style", 15), bg="white").place(x=350, y=230)
        lbl_utype = Label(self.root, text="User Type", font=("goudy old style", 15), bg="white").place(x=790, y=230)
        ...
        txt_pass = Entry(self.root, textvariable=self.var_pass_id, font=("goudy old style", 15), bg="lightyellow").place(x=500, y=230, width=180)
        cmb_utype = ttk.Combobox(self.root, textvariable=self.var_utype_id, values=("Admin", "Employee"), state="readonly", justify=CENTER, font=("goudy old style", 15))
        cmb_utype.place(x=750, y=230, width=180)
        cmb_utype.current(0)

        # Row 4
        lbl_address = Label(self.root, text="Address", font=("goudy old style", 15), bg="white").place(x=50, y=270)
        lbl_salary = Label(self.root, text="Salary", font=("goudy old style", 15), bg="white").place(x=680, y=270)

        self.txt_address = Text(self.root, font=("goudy old style", 15), bg="lightyellow")
        self.txt_address.place(x=150, y=270, width=300, height=60)
        txt_salary = Entry(self.root, textvariable=self.var_salary_id, font=("goudy old style", 15), bg="lightyellow").place(x=750, y=270, width=180)
    ...
    def get_data(self, ev):
        ...
        self.var_gender_id.set(row[3])
        self.var_contact_id.set(row[4])
        self.var_dob_id.set(row[5])
        self.var_doj_id.set(row[6])
        self.var_pass_id.set(row[7])
        self.var_utype_id.set(row[8])
        self.txt_address.delete('1.0', END)
        self.txt_address.insert(END, row[9])
        self.var_salary_id.set(row[10])
    ...
    def clear(self):
        ...
        self.var_gender_id.set("Select")
        self.var_contact_id.set("")
        self.var_dob_id.set("")
        self.var_doj_id.set("")
        self.var_pass_id.set("")
        self.var_utype_id.set("Admin")
        self.txt_address.delete('1.0', END)
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get() == "Select":
                ...
                # Updated search query with parameterization
